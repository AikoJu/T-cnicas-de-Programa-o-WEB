...
//        return status(200).body(musica.getFoto());
    ...
    @GetMapping("/simples")
    public ResponseEntity<List<MusicaSimplesResponse>>
            getSimples() {
        return status(200).body(repository.findSimples());
    }

    @GetMapping("/resumido")
    public ResponseEntity<List<MusicaResumidaResponse>>
            getResumido() {
        return status(200).body(repository.findResumido());
    }


    @PatchMapping("/{codigo}/mais-ouvinte")
    public ResponseEntity<Void> maisOuvinte(
            @PathVariable Integer codigo) {

        /*if (!repository.existsById(codigo)) {
            return status(404).build();
        }
        repository.aumentarOuvintesEm1(codigo);
        return status(204).build();*/
        var alterados = repository.aumentarOuvintesEm1(codigo);
        ...
        /*
        Integer status = alterados == 1 ? 204 : 404;
